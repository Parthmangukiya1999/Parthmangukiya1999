# Hey there (●'◡'●)
# Parth Mangukiya

## About Me
I am an engineering student specializing in **Industrial Information Technology** at **LAB University of Applied Sciences**.  
My academic focus is on becoming an **AI-focused Data Scientist**, with strong practical experience in **IoT systems, data analysis, and backend development**.

My projects combine **real-world data collection**, **structured analysis**, and **system-level thinking**, following a clear learning roadmap from fundamentals to applied AI systems.

---

## Learning Roadmap (AI-Focused Data Scientist)

My learning path follows a structured progression:

1. **Programming**
   - Python, SQL, basic R
   - Applied in data analysis and backend systems

2. **Mathematics for AI**
   - Statistics, linear algebra, calculus
   - Used for understanding models and data behavior

3. **Data Analysis**
   - Data cleaning, feature engineering, exploratory analysis
   - Hands-on work with large real-world datasets

4. **Machine Learning**
   - Regression, classification, and introductory deep learning concepts
   - Focus on practical understanding rather than black-box usage

5. **Data Collection**
   - APIs, web data, sensor-based data (IoT)
   - Emphasis on data quality and reliability

6. **Visualization**
   - Clear reporting using charts, dashboards, and summaries
   - Focus on communication of insights


---

## Selected Academic & Technical Projects

### Machine Learning & Data Science Projects
### Population Analysis & Machine Learning Project (Finland Population Dataset + Helsinki Trend Analysis)

A machine learning and data analytics project focused on analyzing long-term population trends, demographic changes, and regional growth patterns using real-world Finnish population datasets, with a specific focus on Helsinki district-level analysis.

**Focus areas**
- Data preprocessing and cleaning  
- Exploratory Data Analysis (EDA)  
- Feature engineering and trend analysis  
- Statistical analysis of population movement, birth, and death trends  
- Regional demographic comparison across Helsinki districts  
- Regression and classification model experimentation  
- Pattern recognition and insight generation for urban development  
- Visualization of demographic insights and reporting  

**Technologies:**  
Python, Pandas, NumPy, Matplotlib, Scikit-learn, SQL, Excel, Jupyter Notebook, SuperComputer

### IoT Weather Station – Raspberry Pi Pico / Pico W
An academic IoT project that collects temperature and humidity data using sensors, displays real-time values on an OLED screen, and sends data to a backend or cloud service using HTTP and JSON.

**Focus areas**
- Embedded systems and sensor integration  
- Data collection pipeline (sensor → device → server)  
- Practical IoT system design  

**Technologies:**  
Raspberry Pi Pico / Pico W, C/C++, DHT sensors, OLED (SH1107), Wi-Fi, REST APIs

---

### EV Charging Data Analytics (Academic Project)
A large-scale data analysis project exploring EV charging behavior, efficiency patterns, and regional usage trends.

**Focus areas**
- Data cleaning and validation  
- Exploratory data analysis  
- Visualization and reporting for decision-making  

**Technologies:**  
Python, SQL, Excel, Power BI, Jupyter Notebook

---

### Soul Project – Web Application (Backend Focus)
A full-stack academic project focused on backend development, authentication, and data management.

**Focus areas**
- REST API design  
- Database modeling  
- Secure user management  

**Technologies:**  
Node.js, Express, MongoDB / Cosmos DB, Azure, DevOps

---

## Technical Skills

**Programming**
- Python, C/C++, JavaScript, SQL

**Data & AI**
- Data analysis and preprocessing
- Basic machine learning concepts
- Statistics for data science

**IoT & Embedded Systems**
- Raspberry Pi Pico / Pico W
- Sensors and microcontroller programming
- IoT data pipelines

**Web & Backend**
- Node.js, Express
- REST APIs
- Databases

**Tools**
- Git & GitHub
- Excel (advanced), Power BI, Jupyter Notebook, VS Code
- PlatformIO / Arduino framework

---

## Education
**LAB University of Applied Sciences**  
Bachelor of Engineering  
Industrial Information Technology

---

## Contact
- LinkedIn: https://www.linkedin.com/in/parth-mangukiya-b84710209/
- Email: parthmangukiya304@gmail.com

---

## GitHub Activity
![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=Parthmangukiya1999&theme=dark&hide_border=false)

---
<be><br>

# 💻 Tech Stack:
![HTML5](https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white) ![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54) ![CSS3](https://img.shields.io/badge/css3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white) ![GithubPages](https://img.shields.io/badge/github%20pages-121013?style=for-the-badge&logo=github&logoColor=white) ![PythonAnywhere](https://img.shields.io/badge/pythonanywhere-%232F9FD7.svg?style=for-the-badge&logo=pythonanywhere&logoColor=151515) ![NumPy](https://img.shields.io/badge/numpy-%23013243.svg?style=for-the-badge&logo=numpy&logoColor=white) ![Matplotlib](https://img.shields.io/badge/Matplotlib-%23ffffff.svg?style=for-the-badge&logo=Matplotlib&logoColor=black) ![Pandas](https://img.shields.io/badge/pandas-%23150458.svg?style=for-the-badge&logo=pandas&logoColor=white) ![TensorFlow](https://img.shields.io/badge/TensorFlow-%23FF6F00.svg?style=for-the-badge&logo=TensorFlow&logoColor=white) ![GitHub Actions](https://img.shields.io/badge/github%20actions-%232671E5.svg?style=for-the-badge&logo=githubactions&logoColor=white) ![GitLab](https://img.shields.io/badge/gitlab-%23181717.svg?style=for-the-badge&logo=gitlab&logoColor=white)


## 🏆 GitHub Trophies
![](https://github-profile-trophy.vercel.app/?username=Parthmangukiya1999&theme=radical&no-frame=false&no-bg=true&margin-w=4)

### ✍️ Random Dev Quote
![](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=radical)

---
[![](https://visitcount.itsvg.in/api?id=Parthmangukiya1999&icon=0&color=0)](https://visitcount.itsvg.in)

## Road Map
<img width="831" height="1056" alt="image" src="https://github.com/user-attachments/assets/5099d363-fe62-46ce-9c07-e9d0e4a59028" />

*This repository serves as my main academic and technical profile.  
Each project repository contains detailed documentation, implementation, and learning outcomes.*

<!-- Proudly created with GPRM ( https://gprm.itsvg.in ) -->
